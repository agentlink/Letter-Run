//
//  LRPositionConstants.h
//  Letter Go
//
//  Created by Gabriel Nicholas on 12/10/13.
//  Copyright (c) 2013 Gabe Nicholas. All rights reserved.
//

static const float zPos_HealthSection = 100;
static const float zPos_GamePlayLayer = 1;
static const float zPos_LetterSection = 90;

static const float zPos_SectionBlock = 95;
static const float zPos_FallingEnvelope = 35;

static const float zPos_SkyLayer = 10;
static const float zPos_MountainLayer = 20;
static const float zPos_HillsLayer = 30;
static const float zPos_GrassLayer = 50;

static const float zPos_Mailman = 40;
static const float zPos_Mailbag = 60;

static const float zPos_PauseButton = 45;

static const float zDiff_Letter_Envelope = .002;
static const float zDiff_Envelope_Envelope = .004;

static const float zDiff_SectionBlock_LetterSection = 10;
static const float zDiff_SelectedBlock_SectionBlock = 10;
